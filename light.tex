...
\begin{align}
\mathrm{commit}(D) &\rightarrow h,\\ \mathrm{prove}(D, j) &\rightarrow \pi, \text{ and} \\
\mathrm{verify}(h, d, j, \pi) \text{ is a boolean}.
\end{align}

The correctness of a Vector Commitment is specified as:
\begin{align}
  \forall D \forall j \mathrm{verify}(\mathrm{commit}(D), D[j], j, \mathrm{prove}(D, j)) = \mathrm{true}
\end{align}
