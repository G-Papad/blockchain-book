...
\begin{proof}
  ...
  In order for $x_1 \neq x_2$, it must be the case that the $y$ input to the adversary $\mathcal{A}$ must have more than one preimage. We'll call these $y$ \emph{ambiguous} images of the hash. So let's consider the subspace $X = \{ x \in \{0, 1\}^{2\kappa}: \exists x' \in \{0, 1\}^\kappa: x \neq x' \land H(x') = H(x) \}$ of those inputs with an ambiguous image. By
  the Pigeonhole Principle, the inputs with unambiguous images must be fewer than $2^\kappa$, so $|X| > 2^{2\kappa} - 2^\kappa = 2^\kappa(2^\kappa - 1)$, which makes it large. We will condition our probabilities on the event that $x_1 \in X$.
  The probability of this event is $\Pr[x_1 \in X] = \frac{|X|}{2^{2\kappa}} = 1 - 2^{-\kappa}$, which is overwhelming. The complementary event has negligible probability $\Pr[x_1 \notin X] = 2^{-\kappa}$.

  Let us examine if $\mathcal{A}$ continues to succeed under this conditioning. For brevity,
  let us denote the event $\textsf{preimage-game}_\mathcal{A}(\kappa)$ by $E$ and the event
  $\textsf{2nd-preimage-game}_{\mathcal{A}'}(\kappa)$ by $E'$.

  Applying the law of total probability, we have that
  \begin{align*}
    \Pr[E] = \Pr[E \mid x_1 \in X] \Pr[x_1 \in X] + \Pr[E \mid x_1 \notin X] \Pr[x_1 \notin X]\,.
  \end{align*}

  Solving for $\Pr[\textsf{preimage-game}_\mathcal{A}(\kappa) \mid x_1 \in X]$, we get
  \begin{align*}
    \Pr[E \mid x_1 \in X] = \frac{\Pr[E] - \Pr[E \mid x_1 \notin X] \Pr[x_1 \notin X]}{\Pr[x_1 \in X]} \geq\\
    \Pr[E] - \Pr[E \mid x_1 \notin X] \Pr[x_1 \notin X] \geq
    \Pr[E] - \Pr[x_1 \notin X] =\\
    \textsf{non-negl}(\kappa) - \textsf{negl}(\kappa) = \textsf{non-negl}(\kappa)\,.
  \end{align*}

  So the adversary $\mathcal{A}$ still has a non-negligible probability of success under this conditioning.

  Next, let us calculate the probability of success of $\mathcal{A}'$. Since the assumption
  was that $\mathcal{A}'$ is successful with negligible probability, for the contradiction
  it suffices to show that $\Pr[E']$ is non-negligible.

  Using the law of total probability, we have that
  \begin{align*}
    \Pr[E'] = \Pr[E' \mid x_1 \in X] \Pr[x_1 \in X] + \Pr[E' \mid x_1 \notin X] \Pr[x_1 \notin X] \geq\\
    \Pr[E' \mid x_1 \in X] \Pr[x_1 \in X]\,.
  \end{align*}

  Since $\Pr[x_1 \in X]$ is non-negligible, it suffices to show that $\Pr[E' \mid x_1 \in X]$ is non-negligible.
  Again let us apply the law of total probability on the event of $\mathcal{A}$ succeeding:

  \begin{align*}
    \Pr[E' \mid x_1 \in X] =\\
    \Pr[E' \mid E \land x_1 \in X] \Pr[E \mid x_1 \in X]
    + \Pr[E' \mid \neg E \land x_1 \in X] \Pr[\neg E \mid x_1 \in X] \geq\\
    \Pr[E' \mid E \land x_1 \in X] \Pr[E \mid x_1 \in X]
  \end{align*}

  Since $\Pr[E \mid x_1 \in X]$ is non-negligible, it suffices to show that $\Pr[E' \mid E \land x_1 \in X]$ is non-negligible. If $E$ is successful, for $E'$ to be successful, it suffices that
  $x_1 \neq x_2$. Since the adversary $\mathcal{A}$ is only given $y$, the conditioning on
  $x_1 \in X$ retains the uniform distribution of $x_2$. Since for each $y \in H(X)$ there
  are at least two $x \in X$ that hash to $y$, the probability of $x_1 \neq x_2$ is at
  least $\frac{1}{2}$. Therefore
  $\Pr[E' \mid E \land x_1 \in X] = \Pr[x_1 \neq x_2 \mid E \land x_1 \in X] \geq \frac{1}{2}$,
  which is non-negligible as desired.
\end{proof}

% TODO: write this proof in the forward direction
% This may allow us to add this comment:
