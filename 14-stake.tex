...
will now be a map from public keys to non-negative integer amounts. The honest majority
...
\begin{definition}[Honest Stake Majority Assumption]
  For a given \emph{stake distribution} $\SD\colon \PKI \longrightarrow \mathbb{N}$
...
\begin{definition}[Target Function]
  A \emph{target function} is a function $\phi\colon [0, 1] \longrightarrow [2^\kappa]$ that maps
  a normalized stake value to a target.
\end{definition}
