...
same reason they are resolved in proof-of-work.

During validation, we demanded that slots in blocks are not in the future. This requires the
...
was missing, the adversary could check a massive number of future slots \emph{in advance} and leverage
...
power takes time. On the other hand, in proof-of-stake, a party can see into the future and
know in which slots she will be a proposer.

Dually, we also demanded that the slots of the blocks in a chain are increasing.
This prevents the adversary from using old blocks in the future. To see this, consider the following
concrete attack: The adversary sleeps for a long period of time and allows the honest parties
...
by including $s$ in the ticketing mechanism, meaning proof-of-work needed to be calculated
\emph{after} the previous block had been mined.

- Adversarial equivocation

\begin{figure}[htbp!]
  \centering
  \includegraphics[width=1 \columnwidth,keepaspectratio]{figures/pos-example.pdf}
  \caption{An example of a proof-of-stake chain.} % TODO: explain it
