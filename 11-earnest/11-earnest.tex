...
\import{./11-earnest/}{algorithms/alg.ro}
    ...
    \begin{algorithmic}[1]
        ...
        \Function{$\mathcal{Z}^{n,t}_{\Pi,\mathcal{A}}$}{$1^\kappa$}
            ...
            \For{$i \gets 1 \text{ to } n - t$}
                ...
                \State $P_i \leftarrow$ {\textsf{new}} $\Pi^H(\mathcal{G})$
            \EndFor
            \State $A \leftarrow${\textsf{new}} $\mathcal{A}^H(\mathcal{G}, n, t)$
            ...
            \EndFor
            \For{$r \gets 1 \text{ to } \textsf{poly}(\kappa)$} \Comment{Number of rounds}
                \State $M \leftarrow \emptyset$
                \For{$i \gets 1 \text{ to } n - t$} \Comment{Execute honest party $i$ for round $r$}
                    \State $Q \leftarrow q$ \Comment{Max honest party oracle queries}
                    \State $M \leftarrow M \cup \{P_i.\textsf{execute}^H(\overline{M}[i])\}$ \Comment{Adversary collects all messages}
                \EndFor
                \State $Q \leftarrow tq$ \Comment{Max adversarial oracle queries}
    ...
    \begin{algorithmic}[1]
     ...
     \Let{\chain}{[\,]}
        \Function{$\textsf{Constructor}$}{$\mathcal{G}'$}
            ...
            \Let{\chain}{[\mathcal{G}]}
        \EndFunction
        \Function{$\textsf{Execute}$}{$M_\textsf{in}$}
            \Let{M_\textsf{out}}{\emptyset}
            \Let{\tilde\chain}{\textsf{maxvalid}(\chain, M_\textsf{in})}
            \If{$\tilde\chain \neq \chain$}
                \Let{M_\textsf{out}}{M_\textsf{out} \cup \{\tilde\chain\}}
            ...
            \If{$B \neq \bot$} \Comment{Successful Mining}
                ...
                \Let{M_\textsf{out}}{M_\textsf{out} \cup \{\chain\}}
            \EndIf
            \State\Return$M$
        \EndFunction
        \Function{$\textsf{Read}$}{}
    ...
    \begin{algorithmic}[1]
      \Function{$\textsf{pow}_{H,T,q}$}{$x, s$}
          ...
          \For{$i \gets 1 \text{ to } q$} \Comment{Number of available queries per party}
              ...
              \If{$H(B) < T$} \Comment{Successful Mining}
                  \State\Return{$B$}
              \EndIf
              \Let{\ctr}{\ctr + 1}
          \EndFor
...
\begin{algorithm}[H]
    ...
    \Function{$\textsf{maxvalid}_{\mathcal{G},\delta(\cdot)}$}{$\overline{C}$}
        ...
        \For{$C \in \overline{C}$} \Comment{Iterate for every chain received through gossip network}
            \If{$\textsf{validate}_{\mathcal{G},\delta(\cdot),\st_0}(C) \land |C| > |C_\textsf{max}|$} \Comment{Longest Chain Rule}
...
\begin{algorithm}[H]
    ...
    \begin{algorithmic}[1]
    \Function{$\textsf{validate}_{\mathcal{G},\delta(\cdot),\st_0}$}{$C$}
        ...
        \For{$B \in C[1{:}]$} \Comment{Iterate for every block in the chain}
            ...
            \If{$H(B) \geq T \lor s \neq h$} \Comment{PoW check and Ancestry check}
                \State\Return$\false$
            \EndIf
            \State st $\leftarrow \delta^*(st, B.x)$ \Comment{Execution layer}
            \If{$st = \bot$}
                \State\Return$\false$
            \EndIf
...
\begin{definition}[Chain Quality]
  A system is said to satisfy \emph{Chain Quality} with parameters $\ell \in \mathbb{N}, \mu \in [0, 1]$ if for all honest parties $P$ and all times $r$, for all $i, j \in \mathbb{N}$ such that $j - i \geq \ell$, we have
  \[\frac{|\mathcal{H}(C^P_r[{i}{:}{j}])|}{j - i} \geq \mu\,.\]
\end{definition}

\begin{definition}[Chain Growth]
  A system is said to satisfy \emph{Chain Growth} with parameters $s \in \mathbb{N}, \tau \in \mathbb{R}^+$ if for all honest parties $P$ and all times $r_1 \leq r_2$ such that $r_2 - r_1 \geq s$, we have
  \[|C^P_{r_2}| - |C^P_{r_1}| \geq \tau (r_2 - r_1)\,.\]
\end{definition}
