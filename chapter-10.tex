...
\begin{table}[htbp!]
    ...
    \begin{tabular}{m{3cm}|m{4cm}|m{6.5cm}}
         ...
         \end{tabular} \\
         \hline
         State update & {Remove consumed outputs and add produced outputs} & \makecell{Update balances \\ $st'[from] \coloneqq st[from] - value$ \\ $st'[to] \coloneqq st[to] + value$} \\
         \hline
         Transaction validation & {Signature, Conservation Law, Inputs exist in $st$.} & {Signature, Sufficient balance, Nonce unique.} \\
         \hline
